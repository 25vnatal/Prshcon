...
| `screen-parish-masses-hub.html` | Parish | Mass Services | ✅ Rebuilt | **Corrected data model per the Mass Services Build Specification**: a Booking can now contain multiple Intentions, each with its own Mass/date, matching MassActivity → MassInstance → MassBooking → MassIntention. Three modules: **Mass Schedule** (config: massType/massName/day/time/recurrence Weekly or One-Time+date-range, booking fee default ₹100, max intentions default 999, booking cut-off default 1hr — all editable per Mass); **Upcoming Masses** (next 4 weeks, intentions shown grouped by type per the spec, excludes anything not Paid/Waived from the sheet, "Stop Bookings" manually closes a specific date even before the automatic cut-off, "Generate Sheet" opens a preview with Download/Print/Send, "Add Office Booking" creates a Parish-Office-sourced booking); **Bookings** (master list across all Masses — Booking #, Mass Date, Intention count, Offered By, Stipend, Payment Status, Source, Created By, Booking Status — with filters for Mass/Payment/Source/Status; click a booking to Edit any Intention inline, Record Payment (Paid/Waived), or view the Receipt). **Fixed the same date-generation bug as the parishioner side** (a Mass whose day-of-week matched "today" produced a duplicate/skipped date) |
| `screen-parishioner-massbooking-request.html` | Parishioner | Book a Mass | ✅ Rebuilt | **Fixed a real date-generation bug**: the "next 3 weeks" loop mishandled the case where a Mass's day-of-week matched "today," producing a duplicate date and skipping one entirely (this is why Sunday Masses looked wrong / extra Masses appeared to land on Saturday) — fixed in this file and in `screen-parish-masses-hub.html`. **Date + Mass merged into one accordion step** (tap a date to expand its Masses inline) instead of two separate screens. **Cart is now collapsed by default**, showing just "N intentions added" + a chevron to expand, plus an always-visible **"Review & Pay →"** shortcut so the "add another" loop can be exited at any time without finishing another full intention. Intention description placeholder/help text still adapts per type. Footer changed from `fixed` to `sticky` within a flex column (and `min-h-screen`→`min-h-[100dvh]`) so the Save & Review button no longer disappears under the mobile keyboard once the cart grows tall. Parish search on Step 1 replaced native `<datalist>` (unreliable on mobile) with a proper custom-rendered results list |
...
- `screen-parishioner-documents-request.html` and `screen-parishioner-massbooking-request.html` — guided wizard flows that already have a fixed footer Back/Continue bar; adding a second fixed bottom element would visually conflict with and dilute the primary task CTA

New Parishioner screens going forward should include the bottom nav unless they're a similar full-screen guided flow, in which case follow the same footer-bar pattern instead.
