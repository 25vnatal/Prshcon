...
| `screen-parish-masses-hub.html` | Parish | Mass Services | ✅ Rebuilt | **Corrected data model per the Mass Services Build Specification**: a Booking can now contain multiple Intentions, each with its own Mass/date, matching MassActivity → MassInstance → MassBooking → MassIntention. Three modules: **Mass Schedule** (config: massType/massName/day/time/recurrence Weekly or One-Time+date-range, booking fee default ₹100, max intentions default 999, booking cut-off default 1hr — all editable per Mass); **Upcoming Masses** (next 4 weeks, intentions shown grouped by type per the spec, excludes anything not Paid/Waived from the sheet, "Stop Bookings" manually closes a specific date even before the automatic cut-off, "Generate Sheet" opens a preview with Download/Print/Send, "Add Office Booking" creates a Parish-Office-sourced booking); **Bookings** (master list across all Masses — Booking #, Mass Date, Intention count, Offered By, Stipend, Payment Status, Source, Created By, Booking Status — with filters for Mass/Payment/Source/Status; click a booking to Edit any Intention inline, Record Payment (Paid/Waived), or view the Receipt) |
| `screen-parishioner-massbooking-request.html` | Parishioner | Book a Mass | ✅ Rebuilt | Corrected to the real journey: **Find Parish** now shows the home parish plus 4 suggested parishes as tappable cards (Our Lady of Sacred Heart, Resurrection, St Mary's Basilica, Sacred Heart), each with real Mass schedules, alongside search for any of the 130-parish directory. Home parish now has 3 Sunday Masses + 2 distinct weekday Masses (morning + evening), all correctly time-sorted (fixed a string-sort bug where "10:00 AM" sorted before "6:30 AM"). Loop of **Date → Mass → Intention** — a persistent **cart strip** now shows every already-added intention (with remove) at the top of these steps so nothing gets lost track of. Intention description placeholder + help text now adapt to the selected type (e.g. "In Memory Of" asks for the departed's name, "Anniversary" asks for the couple). Footer buttons renamed **"Save & Add Another" / "Save & Review"** so it's clear this step both saves the current intention and offers a deliberate next choice, not an ambiguous fork → **Payment** (UPI/Card/Net Banking) → **Confirmation** |
