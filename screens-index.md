...
| `screen-parishioner-documents-request.html` | Parishioner | Documents & Certificates → Request Document | ✅ Done | Guided multi-step flow (doc type → for whom → purpose → supporting docs [conditional] → delivery → payment [conditional] → review/submit); pre-fills family members from the Rodrigues family. Delivery step renamed/clarified (In-app Download / Email / Collect at the Parish / Post / Courier), each with a short explanatory note and method-specific detail capture (email address, mailing address, or parish office pickup info) |
...
| `screen-parish-requests-hub.html` | Parish | Requests hub → Documents & Certificates | ✅ Done | Rebuilt around a mini-workflow checklist per request (Verify → Prepare & upload certificate → Request Approval [optional note] → Priest approval → delivery action specific to the method) instead of a single ambiguous button; tabs are All/Pending/Awaiting Approval/Deliver/Archive; Request More Info and Reject both require a staff-entered message shown to the parishioner; Payment and Document Verification shown as pills. Sacraments/Home Services/Facility Bookings remain "not built yet" placeholders in the same shell |
