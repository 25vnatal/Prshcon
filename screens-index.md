...
| `screen-parishioner-massbooking-request.html` | Parishioner | Book a Mass | ✅ Rebuilt | Corrected to the real journey: **Find Parish** (search the 130-parish directory, defaults to home parish; other parishes show "no Masses configured") → loop of **Date → Mass → Intention** (type from the spec's 6 values: Thanksgiving/Special Intentions/In Memory Of/For the Sick/Anniversary/Birthday) → **Add Another** or **Review** — a single booking can now hold several intentions across different Masses/dates, each removable before payment → **Payment** (UPI/Card/Net Banking only, matching the spec's online methods) → **Confirmation** with booking number and full itemized summary |
